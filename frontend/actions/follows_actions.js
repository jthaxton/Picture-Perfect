...
export const makeFollow = (followee) => (dispatch) => {
  createFollow({id: followee}).then((pictures) => dispatch(receivePictures(pictures)));
}

export const getFollows = () => (dispatch) => fetchFollows().then((follows) => dispatch(receiveAllFollows(follows)));

export const deleteFollow = (follow) => (dispatch) => unFollow(follow).then((pictures) => dispatch(receivePictures(pictures)));
