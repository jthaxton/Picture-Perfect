      ...
      {state.posts.reverse().map(picture => {
        ...
        return (
          ...
          <li key={picture.id}>
            ...
            {state.currentUser.id === picture.user_id ? <input id="sub-button" type="submit" onClick={() => state.deletePicture(picture.id)} value="Delete Post"></input> : <input id="sub-button" type="submit" onClick={() => createFollow(picture.user)} value="Follow User"></input>}
            </div>
            <h2>{picture.title}</h2>
